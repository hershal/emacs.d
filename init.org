...
#+BEGIN_SRC emacs-lisp :tangle yes
  (require 'cl)
  (defun hb/configure-window-system ()
    (if (window-system)
        (progn (scroll-bar-mode 0)
               ...
               (set-fringe-mode 0)))
    (case (window-system)
      (x (hb/configure-os-linux))
      (ns (hb/configure-os-osx))
      (w32 (hb/configure-os-win))))
  (hb/configure-window-system)
#+END_SRC
** portables
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun hb/configure-hydra-movement ()
    ...
    (defhydra hb/hydra-movement (hb-mode-map "M-m" :color red)
      "movement"
      ("j"     (scroll-up-command scroll-amount)    "down")
      ("k"     (scroll-down-command scroll-amount)  "up")
      ("f"     hb/scroll-up-halfpage                "down half")
      ("b"     hb/scroll-down-halfpage              "up half")
      ("SPC"   scroll-up-command                    "down full")
      ("S-SPC" scroll-down-command                  "up full")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun hb/configure-company-backends ()
    ...
    (mapc (lambda (backend) (add-to-list 'company-backends backend))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun hb/configure-helm-bindings ()
    ...
    (after 'helm-files
          ...
          (hb/bind-key "C-c C-d" '(lambda () (interactive) (helm-select-nth-action 0)) helm-find-files-map)))
#+END_SRC
*** advice
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defun hb/configure-multi-term ()
    (setq term-bind-key-alist
          '(("C-n"           . term-send-down)
            ...
            ("C-e"           . term-send-raw)
            ("C-j"           . term-switch-mode)))
    (multi-term-keystroke-setup)
    (add-hook 'term-mode-hook 'hb/configure-multi-term-hook)
    (bind-key "C-j" 'term-switch-mode term-mode-map))
#+END_SRC
*** package
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+END_SRC
** TODO swift
I need to configure more stuff here:
- flycheck
- cask?
- imenu
- [[https://github.com/roupam/fruitstrap][fruitstrap]] [[http://roupam.github.io/#sec-5][configuration here]]
Reference
*** configuration
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun hb/configure-swift ()
    (add-to-list 'flycheck-checkers 'swift))
#+END_SRC
*** package
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package-ensure swift-mode
    :config (hb/configure-swift))
#+END_SRC
** transpose-frame
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package-ensure web-mode
    :config
    (mapc (lambda (extension) (add-to-list 'auto-mode-alist (cons (concat "\\." extension "\\'") 'web-mode)))
...
** swift
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun hb/configure-swift-mode-hook ()
    (setq comment-start "/* "
          comment-end " */"))
  (add-hook 'swift-mode-hook 'hb/configure-swift-mode-hook)
#+END_SRC
* macros
** hb/quotemeta
#+BEGIN_SRC emacs-lisp :tangle yes
...
** TODO :annoy: fix scroll-up-command
